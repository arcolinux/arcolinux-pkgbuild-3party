...
makepkglist="autotiling-git"
  ...
  if [[ $CHOICE == "1" ]] ; then
    ...
    # special packages in chroot

    if [[ $search == "qtscrcpy" ]]; then
      cp -n qtscrcpy-docs*pkg.tar.zst $destiny
      cp -n qtscrcpy-docs*pkg.tar.zst.sig $destiny
    fi
  ...
  # special packages in MAKEPKG

  echo "Copying created files to " $destiny
  echo "#############################################################################################"
  ...
  if [[ $search == "qtscrcpy" ]]; then
    ...
    cp -n qtscrcp*pkg.tar.zst.sig $destiny
  fi

  #take special care to special packages
  if [[ $search == "perl-checkupdates-aur" ]]; then
    cp -n checkupdates*pkg.tar.zst $destiny
    cp -n  checkupdates*pkg.tar.zst.sig $destiny
  fi

  if [[ $search == "droidcam" ]]; then
    gpg --detach-sign v4l2loopback-dc-dkms*pkg.tar.zst
    cp -n v4l2loopback-dc-dkms*pkg.tar.zst $destiny
    cp -n v4l2loopback-dc-dkms*pkg.tar.zst.sig $destiny
  fi

  if [[ $search == "mangohud" ]]; then
    cp -n mango*pkg.tar.zst $destiny
    cp -n mango*pkg.tar.zst.sig $destiny
  fi
  ...
  cp -n $search*pkg.tar.zst "$destination7/$firstLetter/$search1"
  cp -n $search*pkg.tar.zst.sig "$destination7/$firstLetter/$search1"
